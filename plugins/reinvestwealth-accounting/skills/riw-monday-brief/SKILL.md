...
3. **Always print the freshness of the cash figures.** The app reports it per bank connection (per institution, not per account), and it can be missing; a custom statement-upload account has no sync time at all. A cash figure from a feed that last synced five days ago is the single most dangerous thing on this page: if any connection is more than 3 days stale, or its update time is unknown, the brief says so above the cash number, not in a footnote.
...
| Connected accounts: name, type, currency, current balance | The cash block |
| Bank connections: institution, health, last update time | The staleness warning |
| The dashboard summary: cash by currency, window revenue and expenses | A cross-check on the cash and weekly blocks in one call |
| Transactions for the last 8 weeks, with category, amount before tax, date, and whether categorized or pending | Last week's numbers, the trailing average |
| An uncategorized-only transaction listing | The backlog count, directly (it carries a truncation marker when older uncategorized rows may exist beyond the scan window; say so if set) |
...
If the business has more than one entity and the user did not say which, ask once, list them, and stop. Do not brief all of them at once unless asked, and if asked, produce one page each. Transaction reads page at up to 50 rows per call with a cursor, so the 8-week pull is several pages; plan for it.
...
- Each account's balance came from the app, and you checked each institution's connection health and last-update time.
...
**1. Cash on hand.** Total across accounts, then each account on its own line with its balance, and the institution's last-sync time beside its accounts (or "sync time unknown" where the connection does not report one). One line under it: net movement over the week just ended, and whether that is up or down. If a connection is stale, the warning goes here, first.
