...
4. **Everything ties to the app's own reports.** Revenue and expenses tie to the Profit & Loss, balances tie to the Balance Sheet, cash ties to the connected account balances. Prove the tie-outs before you write a page.
...
| Balance Sheet as of the most recent month-end | The balance sheet page |
| Connected accounts: name, type, currency, balance, last-synced time | The cash position |
...
If the business has more than one entity and the user did not say which, ask once, list them, and stop.

**Note what does not exist:** ReInvestWealth has no loan subledger and no accounts-payable subledger. Existing debt payments have to be detected from recurring transaction history and confirmed with the user. Say in the package that the obligations list is derived from the books plus the owner's confirmation, and do not present it as a complete debt schedule.
...
- Every cash balance came from the app with a last-synced time. If any account is more than 3 days stale, tell the user and offer to wait for a sync, because a lender comparing this package to a bank statement will land on exactly that number.
...
**3. Balance sheet.** As of the most recent month-end, straight from the app.

**4. Cash position.** Each connected account with its balance and last-synced time, and the total. If an account the owner mentions is not connected, say the coverage gap out loud rather than presenting a partial total as the whole.
